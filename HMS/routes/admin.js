...
const { isLoggedIn, isAdminOrDoctor, isAuthenticated } = require('../utils/middlewares');
...
router.route("/dashboard")
    .get(isAdminOrDoctor, isLoggedIn("admin"), isAuthenticated, async (req, res) => {
...
router.route("/appointment/create").get(isAdminOrDoctor, isLoggedIn("admin"), async (req, res) => {
    const userType = req.session.userType;
    const doctors = await Doctor.find({});
    const admin = await fetchAdminDetails(req.user.email);
...
router.get("/appointment/manage", isAdminOrDoctor, isLoggedIn("admin"), async (req, res) => {
    const appointments = await Appointment.find({});
    const userType = req.session.userType;
    const admin = await fetchAdminDetails(req.user.email);
...
router.route("/appointment/manage/:id").get(isAdminOrDoctor, isLoggedIn("admin"), async (req, res) => {
    const userType = req.session.userType;
...
router.route("/pharmacy/add").get(isAdminOrDoctor, isLoggedIn("admin"), async (req, res) => {
    const userType = req.session.userType;
    const admin = await fetchAdminDetails(req.user.email);
...
router.get("/pharmacy/manage", isAdminOrDoctor, isLoggedIn("admin"), async (req, res) => {
    const userType = req.session.userType;
    const pharmacies = await Pharmacy.find({});
    const admin = await fetchAdminDetails(req.user.email);
...
router.route("/pharmacy/manage/:id").get(isAdminOrDoctor, isLoggedIn("admin"), async (req, res) => {
    const userType = req.session.userType;
...
router.get("/prescription/add", isAdminOrDoctor, isLoggedIn("admin"), async (req, res) => {
    const userType = req.session.userType;
    const patients = await Patient.find({});
...
router.route("/prescription/add/:id")
...
router.get("/prescription/q", isAdminOrDoctor, isLoggedIn("admin"), async (req, res) => {
    const userType = req.session.userType;
    const prescriptions = await Prescription.find({});
    const patients = await Patient.find({});
    const admin = await fetchAdminDetails(req.user.email);
...
router.get("/prescription/q/:id", isAdminOrDoctor, isLoggedIn("admin"), async (req, res) => {
    const userType = req.session.userType;
...
router.get("/prescription/manage", isAdminOrDoctor, isLoggedIn("admin"), async (req, res) => {
    const userType = req.session.userType;
...
router.route("/prescription/manage/:id").get(isAdminOrDoctor, isLoggedIn("admin"), async (req, res) => {
    const userType = req.session.userType;
    const admin = await fetchAdminDetails(req.user.email);
...
router.get("/lab/tests", isAdminOrDoctor, isLoggedIn("admin"), async (req, res) => {
    const userType = req.session.userType;
...
router.route("/lab/tests/:id").get(isAdminOrDoctor, isLoggedIn("admin"), async (req, res) => {
    const userType = req.session.userType;
    const patient = await Patient.findOne({ patient_id: req.params.id });
...
router.get("/lab/results", isAdminOrDoctor, isLoggedIn("admin"), async (req, res) => {
    const userType = req.session.userType;
...
router.route("/lab/results/:id").get(isAdminOrDoctor, isLoggedIn("admin"), async (req, res) => {
    const userType = req.session.userType;
...
router.get("/patient/vitals", isAdminOrDoctor, isLoggedIn("admin"), async (req, res) => {
    const userType = req.session.userType;
    const labs = await LabReport.find({});
    const admin = await fetchAdminDetails(req.user.email);
    const patients = await Patient.find({ treat_status: "Ongoing" });
...
router.route("/patient/vitals/:id").get(isAdminOrDoctor, isLoggedIn("admin"), async (req, res) => {
    const userType = req.session.userType;
    const lab = await LabReport.findById(req.params.id);
    const admin = await fetchAdminDetails(req.user.email);
...
router.get("/lab/reports", isAdminOrDoctor, isLoggedIn("admin"), async (req, res) => {
    const userType = req.session.userType;
    const labs = await LabReport.find({});
    const admin = await fetchAdminDetails(req.user.email);
    const patients = await Patient.find({ treat_status: "Ongoing" });
...
router.get("/lab/reports/:id", isAdminOrDoctor, isLoggedIn("admin"), async (req, res) => {
    const userType = req.session.userType;
...
router.route("/doctor/add").get(isAdminOrDoctor, isLoggedIn("admin"), async (req, res) => {
    const userType = req.session.userType;
    const admin = await fetchAdminDetails(req.user.email);
...
router.route("/doctor/q").get(isAdminOrDoctor, isLoggedIn("admin"), async (req, res) => {
    const userType = req.session.userType;
    const doctors = await Doctor.find({});
    const admin = await fetchAdminDetails(req.user.email);
...
router.get("/doctor/q/:id", isAdminOrDoctor, isLoggedIn("admin"), async (req, res) => {
    const userType = req.session.userType;
...
router.get("/doctor/manage", isAdminOrDoctor, isLoggedIn("admin"), async (req, res) => {
    const userType = req.session.userType;
    const doctors = await Doctor.find({});
    const admin = await fetchAdminDetails(req.user.email);
...
router.route("/patient/register").get(isAdminOrDoctor, isLoggedIn("admin"), async (req, res) => {
    const userType = req.session.userType;
    const admin = await fetchAdminDetails(req.user.email);
...
router.get("/patient/q", isAdminOrDoctor, isLoggedIn("admin"), async (req, res) => {
    const userType = req.session.userType;
    const patients = await Patient.find({});
    const admin = await fetchAdminDetails(req.user.email);
...
router.get("/patient/q/:id", isAdminOrDoctor, isLoggedIn("admin"), async (req, res) => {
    const userType = req.session.userType;
    const patient = await Patient.findOne({ patient_id: req.params.id });
    const lab = await LabReport.findOne({ patient_id: req.params.id });
    const admin = await fetchAdminDetails(req.user.email);
...
router.get("/patient/manage", isAdminOrDoctor, isLoggedIn("admin"), async (req, res) => {
    const userType = req.session.userType;
    const patients = await Patient.find({});
    const admin = await fetchAdminDetails(req.user.email);
...
router.route("/patient/manage/:id").get(isAdminOrDoctor, isLoggedIn("admin"), async (req, res) => {
    const userType = req.session.userType;
...
router.get("/patient/discharge", isAdminOrDoctor, isLoggedIn("admin"), async (req, res) => {
    const userType = req.session.userType;
    const patients = await Patient.find({});
    const admin = await fetchAdminDetails(req.user.email);
...
router.route("/patient/discharge/:id").get(isAdminOrDoctor, isLoggedIn("admin"), async (req, res) => {
    const userType = req.session.userType;
    const admin = await fetchAdminDetails(req.user.email);
    const patient = await Patient.findOne({ patient_id: req.params.id });
...
router.route("/employee/add").get(isAdminOrDoctor, isLoggedIn("admin"), async (req, res) => {
    const userType = req.session.userType;
    const admin = await fetchAdminDetails(req.user.email);
...
router.get("/employee/q", isAdminOrDoctor, isLoggedIn("admin"), async (req, res) => {
    const userType = req.session.userType;
    const employees = await Employee.find({});
    const admin = await fetchAdminDetails(req.user.email);
...
router.get("/employee/q/:id", isAdminOrDoctor, isLoggedIn("admin"), async (req, res) => {
    const userType = req.session.userType;
    const admin = await fetchAdminDetails(req.user.email);
...
router.get("/employee/manage", isAdminOrDoctor, isLoggedIn("admin"), async (req, res) => {
    const userType = req.session.userType;
    const employees = await Employee.find({});
    const admin = await fetchAdminDetails(req.user.email);
...
router.route("/employee/manage/:id").get(isAdminOrDoctor, isLoggedIn("admin"), async (req, res) => {
    const userType = req.session.userType;
    const employee = await Employee.findById(req.params.id);
    const admin = await fetchAdminDetails(req.user.email);
...
router.get("/records/appointment", isAdminOrDoctor, isLoggedIn("admin"), async (req, res) => {
    const userType = req.session.userType;
    const appointments = await Appointment.find({});
    const admin = await fetchAdminDetails(req.user.email);
...
router.get("/records/appointment/:id", isAdminOrDoctor, isLoggedIn("admin"), async (req, res) => {
    const userType = req.session.userType;
    let app = await Appointment.findById(req.params.id);
...
router.get("/records/patient", isAdminOrDoctor, isLoggedIn("admin"), async (req, res) => {
    const userType = req.session.userType;
    const patients = await Patient.find({});
    const admin = await fetchAdminDetails(req.user.email);
...
router.get("/records/patient/:id", isAdminOrDoctor, isLoggedIn("admin"), async (req, res) => {
    const userType = req.session.userType;
    let patient = await Patient.findOne({ patient_id: req.params.id });
...
router.get("/payroll/add", isAdminOrDoctor, isLoggedIn("admin"), async (req, res) => {
    const userType = req.session.userType;
    const employees = await Employee.find({});
...
router.route("/payroll/add/:id").get(isAdminOrDoctor, isLoggedIn("admin"), async (req, res) => {
    const userType = req.session.userType;
...
router.get("/payroll/manage", isAdminOrDoctor, isLoggedIn("admin"), async (req, res) => {
    const userType = req.session.userType;
    const payrolls = await Payroll.find({});
    const admin = await fetchAdminDetails(req.user.email);
...
router.route("/payroll/manage/:id").get(isAdminOrDoctor, isLoggedIn("admin"), async (req, res) => {
    const userType = req.session.userType;
...
router.get("/payroll/generate", isAdminOrDoctor, isLoggedIn("admin"), async (req, res) => {
    const userType = req.session.userType;
...
router.get("/payroll/generate/:id", isAdminOrDoctor, isLoggedIn("admin"), async (req, res) => {
    const userType = req.session.userType;
...
router.get("/survey", isAdminOrDoctor, isLoggedIn("admin"), async (req, res) => {
    const userType = req.session.userType;
    const admin = await fetchAdminDetails(req.user.email);
